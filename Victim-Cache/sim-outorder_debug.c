 ...
 *   64-bit support as part of the stats and options handler - long executions may not run correctly on a native 32-bit machine.
 ...
 *
 *   Restructured the out-of-order core to support cycle accurate modeling of independent structures for the re-order buffer, issue queue, register file
 ...
 *
 *   Original SimpleScalar 3.0d implementation, without cycle accurate modeling for some processor structures
 *
 * sim-outorder.c:
 *
...
/*
 * victim cache miss handler function
 */
unsigned long long			//latency of block access
dvic_access_fn(mem_cmd cmd,		//access cmd, Read or Write
...
unsigned long long			//latency of block access
ivic_access_fn(mem_cmd cmd,		//access cmd, Read or Write
	md_addr_t baddr,		//block address to access
	unsigned int bsize,		//size of block to access
	cache_blk_t *blk,		//ptr to block in upper level
	tick_t now,			//time of access
	...
	int context_id)			//context_id for the access
{
	 ...
	 * Debugging
	 */
	printf("\t\tdl1_access_fn\n");

/*
 * if l1 data cache miss, first access victim cache
 */
	unsigned long long lat = 4;

	if(cores[contexts[context_id].core_id].cache_dvic && cmd != 1)
	{
		//victim cache hit
		if(cores[contexts[context_id].core_id].cache_dvic->cache_probe(baddr) && cores[contexts[context_id].core_id].cache_dl1->repl_blk_addr)
		{
			/*
			 * Debugging
			 */
			...
			//victim cache hit
			//l1 data cache hit
			cores[contexts[context_id].core_id].cache_dl1->misses--;
			cores[contexts[context_id].core_id].cache_dl1->hits++;
			cores[contexts[context_id].core_id].cache_dl1->victim_hits++;

			//victim block <-> l1 data cache block
			cores[contexts[context_id].core_id].cache_dvic->cache_flush_addr(baddr, now);
			lat += cores[contexts[context_id].core_id].cache_dvic->cache_access(cmd, cores[contexts[context_id].core_id].cache_dl1->repl_blk_addr, context_id, NULL, bsize, now, NULL, NULL);
		...
		//victim cache miss
		//if victim cache is full, LRU block is flushed
		//l1 data cacheblock -> victim cache MRU block
		cores[contexts[context_id].core_id].cache_dvic->cache_access(cmd, cores[contexts[context_id].core_id].cache_dl1->repl_blk_addr, context_id, NULL, bsize, now, NULL, NULL);
		lat += 1;
	}
	...
	else
	{
		...
		if(cmd == Read)
		{
			 ...
			 * Debugging
			 */
			printf("\t\t\tmain_mem->mem_access_latency\n");

			lat += cores[contexts[context_id].core_id].main_mem->mem_access_latency(baddr, bsize, now, context_id);
			return lat;
		}
		else
		{
	...
	int context_id)			//context_id of the access
{
	 ...
	 * Debugging
	 */
	printf("\t\til1_access_fn\n");

/*
 * if l1 instruction cache miss, first access victim cache
 */
	unsigned long long lat = 4;

	if(cores[contexts[context_id].core_id].cache_ivic && cmd != 1)
	{
		//victim cache hit
		if(cores[contexts[context_id].core_id].cache_ivic->cache_probe(baddr) && cores[contexts[context_id].core_id].cache_il1->repl_blk_addr)
		{
			/*
			 * Debugging
			 */
			...
			cores[contexts[context_id].core_id].cache_il1->misses--;
			cores[contexts[context_id].core_id].cache_il1->hits++;
			cores[contexts[context_id].core_id].cache_il1->victim_hits++;

			//victim block <-> l1 instruction cache block
			cores[contexts[context_id].core_id].cache_ivic->cache_flush_addr(baddr, now);
			lat += cores[contexts[context_id].core_id].cache_ivic->cache_access(cmd, cores[contexts[context_id].core_id].cache_il1->repl_blk_addr, context_id, NULL, bsize, now, NULL, NULL);
		...
		//victim cache miss
		//if victim cache is full, LRU block is flushed
		//l1 instruction cache block -> victim cache MRU block
		cores[contexts[context_id].core_id].cache_ivic->cache_access(cmd, cores[contexts[context_id].core_id].cache_il1->repl_blk_addr, context_id, NULL, bsize, now, NULL, NULL);
		lat += 1;
	}
	...
	int context_id)			//context_id of the access
{
	if(cache_il3)
	{
		 ...
		 * Debugging
		 */
		printf("\t\t\t\tcache_il3->cache_access: %d\n");
		//access next level of inst cache hierarchy
...
void sim_reg_options(opt_odb_t *odb)
{
	...
	for(unsigned int i=0;i<cores_at_init_time;i++)
	{
		 ...
		 * victim cache option
		 */
		opt_reg_string(odb, "-cache:dvic",offset,
			"victim data cache config, i.e., {<config>|none}",
			&cores[i].cache_dvic_opt, "dvic:1:64:2:l",
			/* print */TRUE, NULL);

		opt_reg_int(odb, "-cache:dviclat",offset,
			"victim data cache hit latency (in cycles)",
			&cores[i].cache_dvic_lat, /* default */1,
			/* print */TRUE, /* format */NULL);

		opt_reg_string(odb, "-cache:ivic",offset,
			"victim instruction cache config, i.e., {<config>|none}",
			&cores[i].cache_ivic_opt, "ivic:1:64:2:l",
			/* print */TRUE, NULL);

		opt_reg_int(odb, "-cache:iviclat",offset,
			"victim instruction cache hit latency (in cycles)",
			&cores[i].cache_ivic_lat, /* default */1,
			/* print */TRUE, /* format */NULL);
...
void sim_check_options()
{
	...
	for(unsigned int i=0;i<num_cores;i++)
	{
		...
		/*
		 * victim cache latencies
		 */
		if(cores[i].cache_dvic_lat < 1)
		{
			printf("Core %d Victim data cache latency must be greater than zero",i);
			assert(0);
		}

		if(cores[i].cache_ivic_lat < 1)
		{
			printf("Core %d Victim instruction cache latency must be greater than zero",i);
			assert(0);
		}
	...
	for(unsigned int i=0;i<num_cores;i++)
	{
		 ...
		 * use an Victim?
		 */
		if(!mystricmp(cores[i].cache_dvic_opt, "none"))
			cores[i].cache_dvic = NULL;
		else
		{
			if(sscanf(cores[i].cache_dvic_opt, "%[^:]:%d:%d:%d:%c", name, &nsets, &bsize, &assoc, &c) != 5)
				fatal("bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<repl>");
			cores[i].cache_dvic = new cache_t(prepend + name, nsets, bsize, /* balloc */FALSE, /* usize */sizeof(md_addr_t), assoc,
				cache_char2policy(c), dvic_access_fn, /* hit latency */cores[i].cache_dvic_lat);
		}

		if(!mystricmp(cores[i].cache_ivic_opt, "none"))
			cores[i].cache_ivic = NULL;
		else
		{
			if(sscanf(cores[i].cache_ivic_opt, "%[^:]:%d:%d:%d:%c", name, &nsets, &bsize, &assoc, &c) != 5)
				fatal("bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<repl>");
			cores[i].cache_ivic = new cache_t(prepend + name, nsets, bsize, /* balloc */FALSE, /* usize */sizeof(md_addr_t), assoc,
				cache_char2policy(c), ivic_access_fn, /* hit latency */cores[i].cache_ivic_lat);
		}
...
void sim_reg_stats(stat_sdb_t *sdb)
{
	...
	stat_reg_formula(sdb, "sim_inst_rate", "simulation speed (in insts/sec)", std::string("sim_num_insn / sim_elapsed_time").c_str(), NULL);
...
void sim_aux_stats(FILE *stream)
{
	...
	for(unsigned int i=0;i<num_cores;i++)
	{
		...
		/*
		 * register victim cache stats
		 */
		//caches.insert(cores[i].cache_dvic);
		//caches.insert(cores[i].cache_ivic);
...
//this policy works well because branches pass through the machine quicker which works to reduce branch misprediction latencies, and very long latency
//instructions (such loads and multiplies) get priority since they are very likely on the program's critical path
	...
	while(committed < cores[core_num].commit_width)
	{
		...
		if(contexts[context_id].ROB[contexts[context_id].ROB_head].ea_comp)
		{
			...
			if((MD_OP_FLAGS(contexts[context_id].LSQ[contexts[context_id].LSQ_head].op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))
			{
				...
				res_template *fu = cores[core_num].fu_pool->res_get(MD_OP_FUCLASS(contexts[context_id].LSQ[contexts[context_id].LSQ_head].op));
				if(fu && (cores[core_num].write_buf.size() < cores[core_num].write_buf_size))
				{
					...
					if(cores[core_num].cache_dl1)
					{
						 ...
						 * Debugging
						 */
						printf("\n\tcache_dl1->cache_access: Write (commit)\n");
	...
	while((rs = cores[core_num].issue_exec_q_next_event(sim_cycle)))
	{
		...
		if(!all_operands_ready(rs))
		{
			...
			if(cores[core_num].recovery_model_v==core_t::RECOVERY_MODEL_SQUASH)
			{
				...
				//FIXME, shouldn't this test for >= 0?
...
void selection(unsigned int core_num)
{
	...
	for(unsigned int n_issued=0;(it!=cores[core_num].ready_queue.end()) && (n_issued < cores[core_num].issue_width);)
	{
		...
		{
			//This instruction better not be a store.
			assert((MD_OP_FLAGS(rs->op) & (F_MEM|F_STORE)) != (F_MEM|F_STORE));

			//issue the instruction to a functional unit
			if(MD_OP_FUCLASS(rs->op) != NA)
			{
				res_template *fu = cores[core_num].fu_pool->res_get(MD_OP_FUCLASS(rs->op));
				if(fu)
				{
					//reserve the functional unit
					if(fu->master->busy)
					...
					if(rs->in_LSQ && ((MD_OP_FLAGS(rs->op) & (F_MEM|F_LOAD)) == (F_MEM|F_LOAD)))
					{
						...
						if(!load_lat)
						{
							...
							if(cores[core_num].cache_dl1 && valid_addr)
							{
								 ...
								 * Debugging
								 */
								printf("\n\tcache_dl1->cache_access: Read (selection)\n");
...
#define SYSCALL(INST)			((contexts[context_id].spec_mode ? panic("speculative syscall at %lld",sim_cycle) : (void) 0),	sys_syscall(regs, mem, INST))
#define PALCALL(INST)			((contexts[context_id].spec_mode ? panic("speculative palcall at %lld",sim_cycle) : (void) 0),	sys_palcall(regs, mem, INST))
...
md_fault_type decode_and_execute(unsigned int core_num, size_t context_id, md_opcode op, int & out1, int & out2, int & in1, int & in2, int & in3, md_addr_t & target_PC, md_addr_t & addr)
{
	byte_t temp_byte(0);
	half_t temp_half(0);
	word_t temp_word(0);
	...
	regs_t *regs = &contexts[context_id].regs;
	md_inst_t inst = contexts[context_id].IFQ[contexts[context_id].fetch_head].IR;
	regs->regs_PC = contexts[context_id].IFQ[contexts[context_id].fetch_head].regs_PC;
	contexts[context_id].pred_PC = contexts[context_id].IFQ[contexts[context_id].fetch_head].pred_PC;
...
void register_rename(unsigned int core_num)
{
...
#ifdef TPM_THREAD
		if(contexts[disp_context_id].tpm_delay)
		{
			contexts_left.erase(contexts_left.begin() + current_context);
			continue;
		}
...
//						fd_set readfd = contexts[i].readfd;
//						fd_set writefd = contexts[i].writefd;
//						fd_set exceptfd = contexts[i].exceptfd;
//						timeval timeout = contexts[i].timeout;
//						int retval = select(contexts[i].nfds,&readfd,&writefd,&exceptfd,&timeout);
//						if(retval<=0)
//						{
//							contexts[i].next_check = 200000;
//							continue;
//						}
...
#ifdef TPM_THREAD
			contexts[disp_context_id].my_time = sim_cycle;
#endif
		}
		...
		fault = decode_and_execute(core_num, disp_context_id, op, out1, out2, in1, in2, in3, target_PC, addr);
		...
		if(fault != md_fault_none)
		{
			...
			std::cerr << "\tSpec_mode: " << contexts[disp_context_id].spec_mode << std::endl;
...
void dispatch(unsigned int core_num)
{
		...
		if(contexts[context_id].interrupts)
		{
			...
			if(contexts[context_id].interrupts & 0x1)
			{
				...
				while(contexts[context_id].entry_point)
				{
					...
					{
//						std::cerr << "Context(" << context_id << ") has reached its entry point at 0x" << std::hex << contexts[context_id].entry_point << std::dec << std::endl;
		...
		else if(contexts[context_id].fetch_regs_PC >= ld_text_bound)
		{
			if(!mem->mem_translate(contexts[context_id].fetch_regs_PC))
			{
				if(!contexts[context_id].spec_mode)
				{
					std::cerr << "Could not find memory page associated with: 0x" << std::hex << contexts[context_id].fetch_regs_PC << std::dec << std::endl;
		...
		if(do_fetch)
		{
			...
			if(cores[core_num].cache_il1)
			{
				 ...
				 * Debugging
				 */
				printf("\n\tcache_il1->cache_access: Read (fetch)\n");
	...
	mem_t *mem)					//memory space to access
{
	...
	else if(!strcmp(cmd, "res"))
	{
		...
		{
			//dump resource state
			cores[i].fu_pool->res_dump(stream);
		}
	}
...
#if 1
void handle_signal(int context_id)
{
}
#else
void handle_signal(int context_id)
{
	cores[contexts[context_id].core_id].flushcontext(contexts[context_id],sim_num_insn);

	md_addr_t sigcontext_loc = contexts[context_id].regs_R[MD_REG_SP] - 0x298;

	md_gpr_t to_write = 0;
	mem->mem_access_direct(Write,sigcontext_loc + 0x00, &to_write, 8);	//onstack, what else should this be?
	mem->mem_access_direct(Write,sigcontext_loc + 0x08, &contexts[context_id].sigmask, 8);
	mem->mem_access_direct(Write,sigcontext_loc + 0x10, &contexts[context_id].regs_PC, 8);
	mem->mem_access_direct(Write,sigcontext_loc + 0x18, &to_write, 8);	//psl, what else should this be?

	contexts[context_id].regs_R[0] = 0;
	contexts[context_id].regs_R[1] = (md_gpr_t)-1;
	contexts[context_id].regs_R[9] = contexts[context_id].pending_signal;
	contexts[context_id].regs_R[MD_REG_A0] = contexts[context_id].pending_signal;
	contexts[context_id].regs_R[MD_REG_A1] = 0;
	contexts[context_id].regs_R[MD_REG_A2] = contexts[context_id].regs_R[MD_REG_SP] - 0x298;
	contexts[context_id].regs_R[MD_REG_A3] = contexts[context_id].sigaction_array[contexts[context_id].pending_signal];
	contexts[context_id].regs_R[MD_REG_A4] = 0;
	contexts[context_id].regs_R[MD_REG_A5] = 0;
	contexts[context_id].regs_R[26] = 0x3ff800d5b44;	//sigtramp
	contexts[context_id].regs_R[27] = contexts[context_id].regs_R[MD_REG_A3];
//	contexts[context_id].regs_R[28] = 0;	//??
	contexts[context_id].regs_R[MD_REG_SP] -= 0x2b0;
	contexts[context_id].regs_PC = contexts[context_id].regs_R[MD_REG_A3];
	contexts[context_id].regs_NPC = contexts[context_id].regs_R[MD_REG_A3] + 4;
//	uniq?
//	vfp?

	contexts[context_id].pending_signal = 0;
}
#endif

//Fast forward handler
//int ff_context(unsigned int context_id, long long insts, int mode)
int ff_context(unsigned int context_id, long long insts_count, ff_mode_t mode)
{
...
//	int is_write;			//store?
	...
	if(contexts[context_id].pending_signal)
	{
		handle_signal(context_id);
	}

	if(contexts[context_id].sleep)
	{
		contexts[context_id].sleep--;
		return 0;
	}
	...
	if(!(mode & NO_WARMUP))
	{
		 ...
		 * Debugging
		 */
		printf("\n\tcache_il1->cache_access: Read (ff_context)\n");

		cores[contexts[context_id].core_id].cache_il1->cache_access(Read, IACOMPRESS(regs->regs_PC),
			context_id, NULL, ISCOMPRESS(sizeof(md_inst_t)), sim_cycle, NULL, NULL);
		if(cores[contexts[context_id].core_id].itlb)
		{
			cores[contexts[context_id].core_id].itlb->cache_access(Read, IACOMPRESS(regs->regs_PC),
				context_id, NULL, ISCOMPRESS(sizeof(md_inst_t)), sim_cycle, NULL, NULL);
		}
	}
	...
	md_addr_t target_PC(0);			//actual next/target PC address
	int core_num = contexts[context_id].core_id;
...
#include "machine.def.c"
	default:
		...
		fprintf(stderr,"Arguments: A0(%llx) A1(%llx) A2(%llx) A3(%llx) A4(%llx) A5(%llx)\n",regs->regs_R[MD_REG_A0],regs->regs_R[MD_REG_A1],regs->regs_R[MD_REG_A2],regs->regs_R[MD_REG_A3],regs->regs_R[MD_REG_A4],regs->regs_R[MD_REG_A5]);
		panic("attempted to execute a bogus opcode (%x) at %llx at fastfwd: %lld",inst,regs->regs_PC,contexts[context_id].fastfwd_cnt - contexts[context_id].fastfwd_left);
	}

	if(!contexts[context_id].pid)
	{
		if(!(mode & NO_WARMUP))
		{
			contexts[context_id].fastfwd_left--;
		}
		return 0;
	}
	...
	if(fault != md_fault_none)
	{
		...
		md_print_insn(inst,regs->regs_PC,stderr);
		fatal(" -> fault (%d) detected @ 0x%08p at fastfwd: %lld", fault, regs->regs_PC,contexts[context_id].fastfwd_cnt - contexts[context_id].fastfwd_left);
	}
	...
	if(MD_OP_FLAGS(op) & F_MEM)
	{
		...
		if(MD_OP_FLAGS(op) & F_STORE)
		{
			if(!(mode & NO_WARMUP))
			{
				 ...
				 * Debugging
				 */
				printf("\n\tcache_dl1->cache_access: Write (ff_context)\n");

				cores[core_num].cache_dl1->cache_access(Write, (addr&~3), context_id, NULL, 4, sim_cycle, NULL, NULL);
			}
		}
		else if(addr)
		{
			 ...
			 * Debugging
			 */
			printf("\n\tcache_dl1->cache_access: Read (ff_context)\n");

			int latency = cores[core_num].cache_dl1->cache_access(Read, (addr&~3), context_id, NULL, 4, sim_cycle, NULL, NULL);

			if((cores[core_num].recovery_model_v==core_t::RECOVERY_MODEL_SQUASH) && (!(mode & NO_WARMUP)))
			{
				//DO LOAD LATENCY PREDICTION
				pred_hit_L1 = contexts[context_id].load_lat_pred->bpred_lookup(
					/* branch address */addr,
					/* target address */ 0,
					/* opcode */ op,
					/* call? */ FALSE,
				...
				contexts[context_id].load_lat_pred->bpred_update(
					/* branch address */addr,
					/* actual target address */pred_hit_L1,
		...
		{
			cores[core_num].dtlb->cache_access(Read, (addr & ~3), context_id, NULL, 4, sim_cycle, NULL, NULL);
		}
	}
	...
	{
		md_addr_t pred_PC = contexts[context_id].pred->bpred_lookup(
			/* branch address */regs->regs_PC,
			/* target address */0,
			/* opcode */op,
			/* call? */MD_IS_CALL(op),
	...
	//check for DLite debugger entry condition
	if(dlite_check_break(contexts[context_id].dlite_evaluator, regs->regs_NPC, is_write ? ACCESS_WRITE : ACCESS_READ, addr, sim_num_insn, sim_num_insn))
		contexts[context_id].dlite_evaluator->dlite_main(regs->regs_PC, regs->regs_NPC, sim_num_insn);
...
#ifdef TPM_THREAD
	if(contexts[context_id].tpm_delay)
	{
		unsigned long long reduce = std::min(contexts[context_id].tpm_delay, (unsigned long long)contexts[context_id].fastfwd_left);
		contexts[context_id].tpm_delay -= reduce;
		contexts[context_id].fastfwd_left -= reduce;
	}
#endif
...
//			fprintf(stderr, "File %s is using fast-forward count from command line (%lld, %lld)\n", contexts[i].filename.c_str(), contexts[i].fastfwd_left,contexts[i].fastfwd_cnt);
		}
	}
	else
	{
//		for(int i=0;i<num_contexts;i++)
//		{
//			fprintf(stderr, "File %s is using fast-forward count from .arg file (%lld, %lld)\n", contexts[i].filename.c_str(), contexts[i].fastfwd_left,contexts[i].fastfwd_cnt);
//		}
	}
			...
			if(contexts[i].interrupts)
			{
				...
				if(contexts[i].interrupts & 0x1)
				{
					...
					while(contexts[i].entry_point)
					{
						...
						{
//							std::cerr << "Context(" << i << ") has reached its entry point: 0x" << std::hex << contexts[i].entry_point << std::dec << std::endl;
...
//					}
	...
	for(unsigned int i=0;i<cores.size();i++)
	{
		...
		/*
		 * init pipeline of victim
		 */
		if(cores[i].cache_dvic){
			cores[i].cache_dvic->reset_cache_stats();
		}

		if(cores[i].cache_ivic){
			cores[i].cache_ivic->reset_cache_stats();
		}
...
#ifdef TPM_THREAD
			if(contexts[i].tpm_delay)
			{
				halted++;
				contexts[i].tpm_delay--;
			}
			if(!contexts[i].pid)
			{
				halted++;
...
#ifdef TPM_THREAD
		if(halted == contexts.size())
		{
			unsigned long long adjustby = contexts[0].tpm_delay;
			size_t index = 0;
//			if(!contexts[0].pid)
			while(!contexts[index].pid)
			{
				index++;
//				adjustby = contexts[1].tpm_delay;
//				index = 1;
			}
			adjustby = contexts[index].tpm_delay;
			assert(contexts[index].pid);

			for(size_t i=0;i<contexts.size();i++)
			{
				if(contexts[i].pid && (contexts[i].tpm_delay < adjustby))
				{
					adjustby = contexts[i].tpm_delay;
//					index = i;
				}
			}
			if(adjustby > 500)
		...
		if(contexts[current_context].pending_signal)
		{
			handle_signal(current_context);
		}
...
#ifdef TPM_THREAD
	std::cerr << "Simulated seconds: " << (double)sim_cycle / (double)2794000000 << std::endl;
#endif
}
