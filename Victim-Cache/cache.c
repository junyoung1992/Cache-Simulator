...
void cache_t::cache_config(FILE *stream)
{
	...
	printf("cache: %s: %d sets, %d byte blocks, %d bytes user data/block\n",
		name.c_str(), nsets, bsize, usize);
	printf("cache: %s: %d-way, `%s' replacement policy, write-back\n",
		name.c_str(), assoc, policy == LRU ? "LRU"
		: policy == Random ? "Random" : policy == FIFO ? "FIFO" : (abort(), ""));
}
...
{
	if(!strcmp((char*)this->name.c_str(), "Core_0_dvic") || !strcmp(this->name.c_str(), "Core_0_ivic") \
 ...
 */
	if(victim_hits) {
		fprintf(stream,"%s.victim_hits          %lld # total number of misses\n",                name.c_str(), victim_hits);
		fprintf(stream,"%s.total_misses         %lld # victim_hits + misses\n",                name.c_str(), total_misses);
	}
...
//	stat_reg_formula(sdb, buf, "MPKI (Misses per thousand instructions)", buf1, NULL);
	...
	printf(",%lld,%f,%lld,%lld,%lld,%f\n", misses+hits, (double)misses/(double)accesses, misses, victim_hits, total_misses, (double)victim_hits/(double)total_misses);
}
	...
	md_addr_t *repl_addr)				//for address of replaced block
{
	...
	this->repl_blk_addr = CACHE_MK_BADDR(this, repl->tag, set);		//cache block address to be replaced. only used for victim cache
