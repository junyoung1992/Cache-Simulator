...
dstr_access_fn(mem_cmd cmd,		//access cmd, Read or Write
...
unsigned long long			//latency of block access
istr_access_fn(mem_cmd cmd,		//access cmd, Read or Write
	md_addr_t baddr,		//block address to access
	unsigned int bsize,		//size of block to access
	cache_blk_t *blk,		//ptr to block in upper level
	tick_t now,			//time of access
	...
	int context_id)			//context_id for the access
{
	...
	if(cores[contexts[context_id].core_id].cache_dstr && cmd == Read) {
		if(lat = cores[contexts[context_id].core_id].cache_dstr->buffer_access(baddr, context_id, NULL, bsize, now)) {
			// hit
			cores[contexts[context_id].core_id].cache_dl1->misses--;
			cores[contexts[context_id].core_id].cache_dl1->stream_hits++;
	...
	int context_id)			//context_id of the access
{
	...
	if(cores[contexts[context_id].core_id].cache_istr && cmd == Read) {
		if(lat = cores[contexts[context_id].core_id].cache_istr->buffer_access(baddr, context_id, NULL, bsize, now)) {
			// hit
			cores[contexts[context_id].core_id].cache_il1->misses--;
			cores[contexts[context_id].core_id].cache_il1->stream_hits++;
...
void sim_reg_options(opt_odb_t *odb)
{
	...
	for(unsigned int i=0;i<cores_at_init_time;i++)
	{
		 ...
		 * Stream buffer
		 */
		opt_reg_string(odb, "-cache:dstr",offset,
			"l1 data stream buffer config, i.e., {<config>|none}",
			&cores[i].cache_dstr_opt, "dstr:16:64:1:l",
			/* print */TRUE, NULL);

		opt_reg_int(odb, "-cache:dstrlat",offset,
			"l1 data stream buffer hit latency (in cycles)",
			&cores[i].cache_dstr_lat, /* default */1,
			/* print */TRUE, /* format */NULL);

		opt_reg_string(odb, "-cache:istr",offset,
			"l1 inst stream buffer config, i.e., {<config>|dl1|dl2|none}",
			&cores[i].cache_istr_opt, "istr:16:64:1:l",
			/* print */TRUE, NULL);

		opt_reg_int(odb, "-cache:istrlat",offset,
			"l1 instruction stream buffer hit latency (in cycles)",
			&cores[i].cache_istr_lat, /* default */1,
			/* print */TRUE, /* format */NULL);
...
void sim_check_options()
{
	...
	for(unsigned int i=0;i<num_cores;i++)
	{
		 ...
		 * Stream buffer
		 */
		if(cores[i].cache_dstr_lat < 1)
		{
			printf("Core %d L1 data stream buffer latency must be greater than zero",i);
			assert(0);
		}

		if(cores[i].cache_istr_lat < 1)
		{
			printf("Core %d L1 instruction stream buffer latency must be greater than zero",i);
			assert(0);
		}
	...
	for(unsigned int i=0;i<num_cores;i++)
	{
		...
		else //dl1 is defined
		{
			 ...
			 * Stream buffer
			 */
			if(!mystricmp(cores[i].cache_dstr_opt, "none")) {

			} else {
				if(sscanf(cores[i].cache_dstr_opt, "%[^:]:%d:%d:%d:%c", name, &nsets, &bsize, &assoc, &c) != 5)
				fatal("bad l1 D-stream buffer parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");

				cores[i].cache_dstr = new cache_t(prepend + name, nsets, bsize, /* balloc */FALSE, /* usize */0, assoc, cache_char2policy(c),
					dstr_access_fn, /* hit lat */cores[i].cache_dstr_lat);
			}
		...
		else //il1 is defined
		{
			 ...
			 * Stream buffer
			 */
			if(!mystricmp(cores[i].cache_istr_opt, "none")) {
			
			} else {
				if(sscanf(cores[i].cache_istr_opt, "%[^:]:%d:%d:%d:%c", name, &nsets, &bsize, &assoc, &c) != 5)
				fatal("bad l1 I-stream buffer parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");

				cores[i].cache_istr = new cache_t(prepend + name, nsets, bsize, /* balloc */FALSE, /* usize */0, assoc, cache_char2policy(c),
					istr_access_fn, /* hit lat */cores[i].cache_istr_lat);
			}
...
void sim_aux_stats(FILE *stream)
{
	...
	for(unsigned int i=0;i<num_cores;i++)
	{
		 ...
		 * Stream buffer
		 */
		caches.insert(cores[i].cache_istr);
		caches.insert(cores[i].cache_dstr);
...
//					}
	...
	for(unsigned int i=0;i<cores.size();i++)
	{
		 ...
		 * Stream buffer
		 */
		if(cores[i].cache_istr){
			cores[i].cache_istr->reset_cache_stats();
		}
		if(cores[i].cache_dstr){
			cores[i].cache_dstr->reset_cache_stats();
		}
